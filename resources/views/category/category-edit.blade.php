...
@section('content')
    ...
    @php
    $parent_name = $items->parent == 0 ? "none" : DB::table('categories')->where('id', $items->parent)->value('name'); 
    @dd($parent_name)
@endphp --}}
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="card">
                            ...
                            <form class="needs-validation" action="{{ route('category-updates') }}" method="POST">
                                @csrf
                                <div class="card-body">
                                    <div class="row">
                                        <div class="col-lg-6">
                                        ...
                                        <div class="col-lg-6">
                                            <div class="form-group">
                                                ...
                                                <select name="parent"
                                                    ...
                                                    @foreach ($categoriesData as $categoryId => $cateData)
                                                        <option value="{{ $categoryId }}" {{$categoryId == $category->parent ? 'selected' : '' }}>{{ $cateData }}</option>
                                                    @endforeach
                                                </select>
                                                @error('parent')
                                                    <span class="invalid-feedback text-danger">{{ $message }}</span>
